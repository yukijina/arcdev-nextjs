...
export default function Estimate() {
  ...
  const estimateDisabled = () => {
    ...
    const emptySelections = questions.filter(question => question.title !== "which features do you expect to use?")
      ...
      const featureSelected = questions.filter(question => question.title === "Which features do you expect to use?").map(question => question.options.filter(option => option.selected)).filter(selections => selections.length > 0);
